# #!/bin/bash
#  Tests streamlet error at S2
# Execution should not save anything and the client should keep retrying
...
populateRedis() {
    ...
    redis-cli SET policy:P1 "{\"id\":\"P1\",\"system\":\"system\",\"scope\":\"*\",\"stream\":\"*\",\"pipeline\":[{\"streamlet\":{\"id\":\"io.nexus.streamlets.functions.ErrorStreamlet\",\"executeOn\":\"ALL\",\"hardware\":\"GPU\",\"partitionLocality\":true,\"transformsContent\":false,\"dataRouting\":false},\"region\":\"CLOUD\",\"arguments\":[]}],\"storage\":[]}" || { echo "Failed to set policy"; return 1; }

    redis-cli SET streamletdescriptor:io.nexus.streamlets.functions.ErrorStreamlet "{\"id\":\"io.nexus.streamlets.functions.ErrorStreamlet\",\"executeOn\":\"ALL\",\"hardware\":\"GPU\",\"partitionLocality\":true,\"transformsContent\":false,\"dataRouting\":false}" || { echo "Failed to set streamlet"; return 1; }
    ...
    redis-cli SET streamletcode:io.nexus.streamlets.functions.ErrorStreamlet "$(cat $PROJECT_DIR/src/main/java/io/nexus/streamlets/functions/ErrorStreamlet.java)" || { echo "Failed to set streamlet code"; return 1; }
...
echo "============================================"
echo " Failure Test: Streamlet Error"
...
echo -n "THIS IS THE STREAMLET ERROR TEST" > /tmp/test_data.txt
...
if [ -f "/tmp/blobstore/test-metadata/scope/stream/test.txt" ]; then
    echo "  Data stored - Validation failed as the flow should not save to storage"
    STORED_CONTENT=$(cat /tmp/blobstore/test-metadata/scope/stream/test.txt 2>/dev/null)
    echo "  Stored content: $STORED_CONTENT"
else
    echo "  Data NOT stored - Validation succeeded"
fi
