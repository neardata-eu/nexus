...
echo "============================================"
echo " Failure Test: Worker Crash"

echo "\n[1/3] Starting EDGE Nexus instance on port 8181..."
...
echo "\n[2/3] Starting CLOUD Nexus instance on port 8182..."
...
echo "\n[Wait] Waiting for instances to start (~15s)..."
sleep 15
...
if [ -f "/tmp/blobstore/test-metadata/scope/stream/test.txt" ]; then
    ...
    STORED_CONTENT=$(cat /tmp/blobstore/scope/stream/test.txt 2>/dev/null)
