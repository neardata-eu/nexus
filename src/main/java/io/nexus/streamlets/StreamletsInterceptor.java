...
public class StreamletsInterceptor extends ForwardingBlobStore {
    ...
    public String putBlob(String containerName, Blob blob, PutOptions putOptions) {
        ...
        // 4. Routing the request to the next Nexus Swarmlet or just allowing the S3Proxy to store the data in the final
        ...
        if (nextSwarmletEndpoint != null) {
            ...
            // 5. Reply to the client once the storage log storage completes, and forward the operation to the next stage.
    ...
    private void tryProcessPut(Policy policy, Region currentRegion, Hardware availableHardware,
    ...
    private void tryProcessGet(Policy policy, Region currentRegion, Hardware availableHardware,
                               String containerName, Blob blob, GetOptions getOptions) {
        if (policy.canSwarmletExecuteStreamlets(currentRegion, availableHardware)) {
            // We are in the right Swarmlet, process the request.
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                if (responseCode >= 200 && responseCode < 300) {
                    ...
                    Blob blob = super.blobBuilder(blobName)
                            .payload(inputStream)
                            .contentLength(100L)
