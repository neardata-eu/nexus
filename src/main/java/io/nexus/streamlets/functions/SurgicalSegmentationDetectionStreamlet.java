...
public class SurgicalSegmentationDetectionStreamlet extends ByteStreamlet {

    private static final Logger logger = LoggerFactory.getLogger(SurgicalSegmentationDetectionStreamlet.class);
    private static final String INFERENCE_KEY = "surgical-segmentation-detection";
    private static final AtomicReference<AbstractAIModelInference<Image, Classifications>> modelHelper = new AtomicReference<>();
    private final String name = "SURGICAL_SEGMENTATION_DETECTION_BYTE";

    public SurgicalSegmentationDetectionStreamlet() {
        if (modelHelper.get() == null) {
            boolean useCuda = AbstractAIModelInference.isCudaAvailable();
            String modelName = "model_segmentation_torchscript.pt";
            String synset = AbstractAIModelInference.getCurrentModelPath() + "/nct_segment_labels.txt";
            ...
            modelHelper.compareAndSet(null, new AbstractAIModelInference<>(
                    ...
                    new SurgicalPhaseTranslator(args, synset),
    ...
    protected void processPutBytes(StreamletIO dataStreams, StreamletContext context) {
        try (InputStream in = dataStreams.input();
            ...
            logger.info("Running inference on surgical segmentation detection model");

            Classifications results = modelHelper.get().detectObjects(imageBytes);
            List<String> detectedSegments = results.items().stream()
                    .filter(item -> item.getProbability() >= 0.5)
                    .map(Classifications.Classification::getClassName)
                    .collect(Collectors.toList());

            logger.info("Possible Segments: {}", detectedSegments);
            context.putUserMetadata(INFERENCE_KEY, String.join(",", detectedSegments));
    ...
    protected void processGetBytes(StreamletIO dataStreams, StreamletContext context) {
        throw new UnsupportedOperationException("GET not supported for phase detection.");
    }
    ...
    public static class SurgicalPhaseTranslator implements Translator<Image, Classifications> {
        ...
        public SurgicalPhaseTranslator(Map<String, Object> args, String synsetFile) {
