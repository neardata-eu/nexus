...
public class SurgicalPhaseDetectionStreamlet extends ByteStreamlet {

    private static final Logger logger = LoggerFactory.getLogger(SurgicalPhaseDetectionStreamlet.class);
    private static final String INFERENCE_KEY = "surgical-phase-detection";
    private static final AtomicReference<AbstractAIModelInference<Image, Classifications>> modelHelper = new AtomicReference<>();
    private final String name = "SURGICAL_PHASE_DETECTION_BYTE";

    public SurgicalPhaseDetectionStreamlet() {
        if (modelHelper.get() == null) {
            boolean useCuda = AbstractAIModelInference.isCudaAvailable();
            String modelName = "model_phase_torchscript.pt";
            String synset = AbstractAIModelInference.getCurrentModelPath() + "/nct_phase_labels.txt";
    ...
    protected void processPutBytes(StreamletIO dataStreams, StreamletContext context) {
        try (InputStream in = dataStreams.input();
            ...
            logger.info("Running inference on surgical phase detection model");

            Classifications results = modelHelper.get().detectObjects(imageBytes);
            List<String> detectedPhases = results.items().stream()
                    .filter(item -> item.getProbability() >= 0.5)
                    .map(Classifications.Classification::getClassName)
                    .collect(Collectors.toList());

            logger.info("Possible Phases: {}", detectedPhases);
            context.putUserMetadata(INFERENCE_KEY, String.join(",", detectedPhases));
