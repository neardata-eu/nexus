    ...
    try:
        ...
        for line in iter(process.stdout.readline, ''):
            ...
            if re.search(r" Writing test result", line):
                ...
                print(f"Captured log: {captured_line}.\nWaiting 1 minute for OMB and Pulsar to finish their processes...")
                time.sleep(2*60)
                process.terminate()
                break
        time.sleep(5)
...
def report(bucket):
    count = 0
    total = 0
    pulsar_file_ext = '-index'
    resource = get_resource()
    iterator = resource.Bucket(bucket).objects.all()
    for obj in iterator:
        if pulsar_file_ext not in obj.key:
            count = count + 1
...
def record(output, results_file, optional_dir):
    ...
    try:
        if not os.path.exists(f"experiments/results/pulsar/{optional_dir}/"):
            os.makedirs(f"experiments/results/pulsar/{optional_dir}/")
            print(f"Created results directory: {f'experiments/results/{optional_dir}/'}")
        results_directory = f"experiments/results/pulsar/{optional_dir}/{results_file}"
    ...
    try:
        recorded_ratios = f"experiments/results/pulsar/{optional_dir}/recorded_ratios.json"
