    ...
    try:
        ...
        for line in iter(process.stdout.readline, ''):
            ...
            if re.search(r" Writing test result", line):
                ...
                print(f"Captured log: {captured_line}.\nWaiting for OMB and Kafka to finish their processes...")
                time.sleep(60)
                process.terminate()
                break
        time.sleep(5)
...
def record(output, results_file, optional_dir):
    ...
    try:
        if not os.path.exists(f"experiments/results/{optional_dir}/"):
            os.makedirs(f"experiments/results/{optional_dir}/")
            print(f"Created results directory: {f'experiments/results/{optional_dir}/'}")
        results_directory = f"experiments/results/{optional_dir}/{results_file}"
    ...
    try:
        recorded_ratios = f"experiments/results/{optional_dir}/recorded_ratios.json"
...
if __name__ == '__main__':
    ...
    parser.add_argument("--system", "-s", action="store", default=None)
